	...
	public static void main(String[] args) {
		...
		while (sair == false) {
			...
			if (operacao == 1) {
				...
				funcionario.setNomeFuncionario(Teclado.lerTexto("Digite o nome do funcionário: "));
				//funcionario.salarioFuncionario = Teclado.lerDouble("Digite o salário do funcionário: "); Questão 40 impõe salário inicial.
				...
				int codigoFunc = Teclado.lerInt("Selecione o código do funcionário.");
					if (codigoFunc >= 0 && codigoFunc <= somaFunc){
						funcionarios.get(codigoFunc - 1).setNomeFuncionario(Teclado.lerTexto("Digite o novo nome do funcionário " + codigoFunc + "."));
						funcionarios.get(codigoFunc - 1).setSalarioFuncionario(Teclado.lerDouble("Digite o novo salário do funcionário " + codigoFunc + "."));
					}
					else {
				...
				for (Funcionario todosFunc : funcionarios) {
					System.out.println(FuncionarioService.consultaFunc(todosFunc) + "\n");				
				}			
			}
			else if (operacao == 4) {
